...
import numpy as np
import torch
from project_ai.v3.engine.const import MPS_UNSUPPORT_MODELS
        ...
        if model_md5:
            ...
            if model_md5 == _md5:
                ...
                try:
                    ...
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, wrong model deleted. Please restart ProjectAI V3."
                        f"If you still have errors, please try download model manually first https://lama-cleaner-docs.vercel.app/install/download_model_manually.\n"
                    )
                except:
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, please delete {cached_file} and restart ProjectAI V3."
                    )
                exit(-1)
...
def handle_error(model_path, model_md5, e):
    ...
    if _md5 != model_md5:
        try:
            ...
            logger.error(
                f"Model md5: {_md5}, expected md5: {model_md5}, please delete {model_path} and restart ProjectAI V3."
            )
    else:
        logger.error(
